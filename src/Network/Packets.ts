...
export class Packets {
    ...
    preseenceUpdate(state: OnlineState) {
        new Packet("subscriber settings update", {
            state: {
                state: state,
            },
        });
    ...
    messagePacket(id: number, isGroup: boolean, msg: any, mimeType: string, flightId?: string) {
    ...
    MessageHistory(id: number, from: Date, group: boolean, chronological: boolean) {
        return new Packet(
            group ? "message group history list" : "message private history list",
            group
                ? {
                      id: id,
                      timestapBegin: 1,
