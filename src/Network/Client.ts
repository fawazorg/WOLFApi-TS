...
export class Client {
    ...
    login(Username: string, Password: string, device?: DeviceType, state?: OnlineState, type?: any) {
        if (!type) type = "email";
        if (!device) device = DeviceType.Web;
        if (!state) state = OnlineState.Online;
    ...
    private _loginsuccess(user: ExtendedClient, state: OnlineState) {
        ...
        this.Connection.on("message send", (data: { body: IMessage }) => {
            if (data.body.originator == this._id) return;
    ...
    private PacketBody(packet: Packet): any {
        ...
        if (packet.body != null || undefined) any.body = packet.body;
        if (packet.headers != null || undefined) any.headers = packet.headers;
        return any;
    }
    ...
    public writePacket(
        ...
        if (!adv) {
            if (!success) this.Connection.emit(packet.command, p);
            else this.Connection.emit(packet.command, p, success);
            return;
        }

        this.Connection.emit(packet.command, p, (data: { code: number; body: any }) => {
            if (debug || this.Debug) console.log("Packet:" + packet.command, data);
            if (data.code == 200) if (success) success(data.body);
            if (data.code != 200) {
