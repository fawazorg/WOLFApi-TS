...
export class PluginInstance {
    ...
    constructor(options: iPluginOptions) {
        ...
        options.authCheck = options.authCheck ? options.authCheck : (u, g) => true;
        options.pmOnly = options.pmOnly ? options.pmOnly : false;
        options.groupOnly = options.groupOnly ? options.groupOnly : false;
        options.description = options.description ? options.description : "PluginInstance";
        options.cmdParser = options.cmdParser ? options.cmdParser : (text) => text;
    }

    static register(command: string, options: iPluginOptions, fn: Function) {
        if (!PluginInstance.plugins) PluginInstance.plugins = new Dictionary<string, PluginInstance>();

        if (!options) options = <iPluginOptions>{};
    ...
    private static onMessage(client: WolfClient, msg: ExtendedMessage) {
        ...
        var val = ps.getBy((k, v) => {
            if (msgText.indexOf(k) === 0) return true;
            for (let c of v.options.aliases) {
                if (msgText.indexOf(c) === 0) return true;
            }
            return false;
        });
