...
export class Actions {
    ...
    async failReasonAdminAction(client: Client, group: ExtendedGroup, target: Number) {
        ...
        await client.info.groupMemberList(group.id, (resp) => {
            for (var i = 0; i < resp.length; i++) {
                if (resp[i].id == target) cachedTarget = resp[i];
                ...
                if (cachedTarget != undefined) {
                    ...
                    if (resp[i].id == client.info.ClientProfile.subscriber.id) {
                        if (
                            resp[i].capabilities == Role.Mod &&
                            cachedTarget.capabilities == (Role.Admin | Role.Mod | Role.Owner)
                        )
                    ...
                    if (i == resp.length) return (this.AdminActionResult.unknownReason = true);
                }
            }
        });
    }
    ...
    async failReasonGroupAction(client: Client, group: ExtendedGroup) {
        ...
        await client.info.requestGroups().then((grp: ExtendedGroup[]) => {
            for (var i = 0; i < grp.length; i++) {
                if (i == grp.length && grp[i].id != group.id) return (this.GroupActionResult.groupNotfound = true);
            }
        });
    }
    ...
    async getExtendedGroup(group: string | number): Promise<ExtendedGroup> {
        return new Promise(async (resolve, reject) => {
            if (this.grp != undefined) this.grp = undefined;
            ...
            if (groupType == GroupType.name) {
                ...
                await this.client.info.requestGroup(group, (resp: ExtendedGroup) => {
                    return resolve(resp);
                });
            }
        });
    ...
    async banUser(group: number, userID: number, callback?: (reason: AdminActionResult) => void) {
        ...
        this.client.writePacket(
            this.Packet.adminAction(group, userID, AdminActionTypes.Ban),
            false,
            false,
            async (data: { code: number; body: any }) => {
                if (data.code != 200) {
                    if (callback) {
                    ...
                    if (callback) callback(this.AdminActionResult);
                }
            },
        );
    }
    ...
    async kickUser(group: number, userID: number, callback?: (reason: AdminActionResult) => void) {
        await this.getExtendedGroup(group);
        this.client.writePacket(
            this.Packet.adminAction(group, userID, AdminActionTypes.Kick),
            false,
            false,
            async (data: { code: number; body: any }) => {
                if (data.code != 200) {
    ...
    async silenceUser(group: number, userID: number, callback?: (reason: AdminActionResult) => void) {
        await this.getExtendedGroup(group);
        this.client.writePacket(
            this.Packet.adminAction(group, userID, AdminActionTypes.Silence),
            false,
            false,
            async (data: { code: number; body: any }) => {
                if (data.code != 200) {
    ...
    async resetUser(group: number, userID: number, callback?: (reason: AdminActionResult) => void) {
        await this.getExtendedGroup(group);
        this.client.writePacket(
            this.Packet.adminAction(group, userID, AdminActionTypes.Reset),
            false,
            false,
            async (data: { code: number; body: any }) => {
                if (data.code != 200) {
    ...
    async modUser(group: number, userID: number, callback?: (reason: AdminActionResult) => void) {
        await this.getExtendedGroup(group);
        this.client.writePacket(
            this.Packet.adminAction(group, userID, AdminActionTypes.Mod),
            false,
            false,
            async (data: { code: number; body: any }) => {
                if (data.code != 200) {
    ...
    async adminUser(group: number, userID: number, callback?: (reason: AdminActionResult) => void) {
        await this.getExtendedGroup(group);
        this.client.writePacket(
            this.Packet.adminAction(group, userID, AdminActionTypes.Admin),
            false,
            false,
            async (data: { code: number; body: any }) => {
                if (data.code != 200) {
    ...
    async joinGroup(groupName: string, password?: string, callback?: (reason: GroupActionResult) => void) {
        await this.getExtendedGroup(groupName).then((val: ExtendedGroup) => {
            this.grp = val;
        });
        await this.client.writePacket(
            this.Packet.groupJoin(groupName, password),
            false,
            false,
            async (data: { code: number; body: any }) => {
                if (data.code != 200) {
                    if (callback) {
                        switch (data.code) {
                            case 4:
                                return (this.GroupActionResult.higherlevel = true);
                            case 100:
                                return (this.GroupActionResult.groupisfull = true);
                            case 101:
                                return (this.GroupActionResult.hitgrouplimit = true);
                            case 105:
                                return (this.GroupActionResult.nolongerexists = true);
                            case 107:
                                return (this.GroupActionResult.banned = true);
                            case 112:
                                return (this.GroupActionResult.restritedtonewusers = true);
                            case 115:
                                return (this.GroupActionResult.groupisclosed = true);
                            case 116:
                                return (this.GroupActionResult.toomanyacounts = true);
                            case 117:
                                return (this.GroupActionResult.groupcanonlybejoinedfromgame = true);
                            case 110:
                                return (this.GroupActionResult.alreadyInGroup = true);
                            default:
                                return (this.GroupActionResult.unknownerror = true);
                        }
                        callback(this.GroupActionResult);
                    }
                } else {
                    this.GroupActionResult.sucess = true;
                    if (callback) callback(this.GroupActionResult);
                }
            },
        );
    }
    ...
    async leaveGroup(groupID: number, callback?: (reason: GroupActionResult) => void) {
        await this.client.info.requestGroups((resp) => {
            for (var i = 0; i < resp.length; i++) if (resp[i].id == groupID) this.grp = resp[i];
        });

        this.client.writePacket(
            this.Packet.groupLeave(groupID),
            false,
            false,
            async (data: { code: number; body: any }) => {
                if (data.code != 200) {
