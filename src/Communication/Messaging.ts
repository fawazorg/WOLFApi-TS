...
export class Messaging {
    ...
    private async _checkMessage(message: Message): Promise<boolean> {
        ...
        return new Promise((resolve, reject) => {
            for (var plugin in PluginInstance.plugins.all()) {
                if (message.text.startsWith(plugin, 1)) return (tempbool = true);
            }
    ...
    async nextMessage(
        ...
        callback?: (resp: ExtendedMessage) => void,
        anyNextMessage?: boolean,
    ): Promise<ExtendedMessage> {
        return new Promise((resolve, reject) => {
            this.client.Connection.on("message send", async (data: { body: IMessage }) => {
                if (
                    data.body.originator != this.client.info.ClientProfile.subscriber.id && anyNextMessage
                        ? true
                        : message.originator == data.body.originator &&
                    ...
                    if (msg.isGroup) msg.group = await this.client.info.requestGroup(msg.recipient);
    ...
    async nextGroupMessage(groupID: number, callback?: (resp: ExtendedMessage) => void): Promise<ExtendedMessage> {
        return new Promise((resolve, reject) => {
            this.client.Connection.on("message send", async (data: { body: IMessage }) => {
                if (
                    data.body.isGroup &&
                    data.body.originator != this.client.info.ClientProfile.subscriber.id &&
                    data.body.recipient == groupID &&
                    (await this._checkMessage(new Message(data.body))) == false
                ) {
                    let msg = new ExtendedMessage(data.body);
    ...
    async groupMessage(
        ...
        if (isImage) {
            this.client.writePacket(
                this.packet.messagePacket(groupID, true, await this.toBuffer(data), "image/jpeg", flightId),
                false,
                false,
                (data) => {
            ...
            this.client.writePacket(
                this.packet.messagePacket(groupID, true, data, "text/plain", flightId),
                false,
                false,
                (data) => {
                    if (completed) completed(data);
    ...
    async privateMessage(
        ...
        if (isImage) {
            this.client.writePacket(
                this.packet.messagePacket(userID, false, await this.toBuffer(data), "image/jpeg", flightId),
                false,
                false,
                (data) => {
                    if (completed) completed(data);
            ...
            this.client.writePacket(
                this.packet.messagePacket(userID, false, data, "text/plain", flightId),
                false,
                false,
                (data) => {
                    if (completed) completed(data);
        ...
        if (isImage) {
            this.client.writePacket(
                this.packet.messagePacket(
                    msg.isGroup == false ? msg.originator : msg.group.id,
                    msg.isGroup,
                    await this.toBuffer(data),
                    "image/jpeg",
                    flightId,
                ),
                false,
