...
export class Stages {
    ...
    async requestAudioSlots(groupID: number, subscribe?: boolean, callback?: (callback: StageInfo[]) => void) {
        this.client.writePacket(this.packet.requestAudioSlots(groupID, subscribe), true, false, (resp: StageInfo[]) => {
    ...
    async broadcastTrack(groupID: number, slotID: number, sdp: string, callback?: (callback) => void) {
        this.client.writePacket(this.packet.broadcast(groupID, slotID, sdp), true, false, (resp) => {
            if (callback) callback(resp);
        });
    }
