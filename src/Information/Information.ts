...
export class Information {
    ...
    async createGroup(group: CGroup) {
        this.client.writePacket(this.Packet.createGroup(group), false, false, (resp) => {
            if (resp.code != 200) this.client.On.Trigger("log", resp);
        });
    }
    ...
    async requestUserExpiredCharms(
        userID: number,
        callback?: (charms: Charms[]) => void,
        offset?: number,
        limit?: number,
    ) {
        this.client.writePacket(
            this.Packet.requestUserExpiredCharms(userID, isNull(offset) ? null : offset, isNull(limit) ? null : limit),
            false,
            false,
            (resp: Charms[]) => {
                if (callback) callback(resp);
    ...
    async requestUserActiveCharms(
        ...
        this.client.writePacket(
            this.Packet.requestUserActiveCharms(userID, isNull(offset) ? null : offset, isNull(limit) ? null : limit),
            false,
            false,
            (resp: Charms[]) => {
                if (callback) callback(resp);
    ...
    async requestTopics(language: Language, callback?: (callback: TopicList) => void, name?: string) {
        this.client.writePacket(
            this.Packet.requestTopics(language, isNull(name) ? null : name),
            true,
            false,
            (resp: TopicList) => {
                if (callback) callback(resp);
    ...
    async requestDiscoverGroups(
        ...
        this.client.writePacket(
            this.Packet.requestDiscoverGroups(
                languageCode,
                maxResults,
                offset,
                isNull(admin) ? false : admin,
                repLevel,
                recipieId,
            ),
            false,
    ...
    conversationList(callback?: (history: Message[]) => void) {
        this.client.writePacket(this.Packet.ConversationHistory(), true, false, (resp: IHistoricalMessage[]) => {
            if (callback) callback(Extensions.fromHistory(resp));
        });
    }
    ...
    messageHistory(
        ...
        callback?: (hist: Message[]) => void,
    ) {
        this.client.writePacket(
            this.Packet.MessageHistory(id, from, group, chronological),
            true,
            false,
            (resp: IHistoricalMessage[]) => {
                if (callback) callback(Extensions.fromHistory(resp));
            },
        );
    }
